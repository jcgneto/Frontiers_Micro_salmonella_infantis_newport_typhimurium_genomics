...
Kmer cluster vs pca shell-gene clustering 
...
d2 <- data2 %>% select(-X1)
d2 <- d2 %>% rename(pca_cluster = cluster)

# merge datasets 

dd <- left_join(d4, d2, on = "id")

# frequencies

dd1 <- dd %>% group_by(pca_cluster, kmer_cluster) %>% count()
dd1
